...
with col_sw2:
    plant_toggle = st.toggle("🥒 Cucumber          🌾 Quinoa", value=True)
    plant_type   = "Cucumber" if plant_toggle else "Quinoa"
    st.markdown(f"<div style='text-align:center; font-weight:700; font-size:1.2rem; color:#1b5e20; margin-top:0.3rem;'>{'🥒 Cucumber' if plant_type == 'Cucumber' else '🌾 Quinoa'}</div>", unsafe_allow_html=True)
